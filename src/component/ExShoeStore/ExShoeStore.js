...
export default class ExShoeStore extends Component {
  ...
  handleDeleteShoe = (idShoeClick) => {
    console.log('delete', idShoeClick);
    let arrCartEdit = this.state.arrCart

    // hàm filter sẽ tạo 1 mảng mới ko chứ object shoe chứa id - với idShoeClick (ko chứa id đã xoá)
    arrCartEdit = arrCartEdit.filter((shoe) => shoe.id != idShoeClick)
    this.setState({
      arrCart: arrCartEdit,
    })
  }
  handleChangeQuantityShoe = (idShoeClick, quantity) => {
    // console.log('change');
    let arrCartEdit = this.state.arrCart;
    // trả về obj đc click
